...
import analmess.tokens
import analmess.words
...
class FrequencyDecoder(freqChar: Map<Char, Double>, private val encodedText: String) {
    ...
    /*fun decodePlus(): String {
        val solver = SolverFactory.newDefault()
        solver.newVar(allPair.size)
        fun varIdx(k: Char, v: Char) = allPair.indexOf(k to v) + 1
        fun idxVar(idx: Int) = allPair[idx - 1]
        // localFreq
        globalFrequency.asReversed().forEach { (space, _) ->
            val words = encodedText.split(space).filter { it.isNotEmpty() }.toMutableSet()
            val estimated = allPair.groupBy { it.first }
                .mapValues { ent -> ent.value.map { it.second }.toMutableList() }
                .toMutableMap()

            run {
                val expIdxs = allPair.filter { it.second == ' ' } //TODO
                    .map { varIdx(it.first, it.second) }
                expIdxs.forEach {
                    solver.addClause(VecInt(listOf(-it).toIntArray()))
                    val (k, v) = idxVar(it)
                    estimated[k]!!.remove(v)
                }
            }
            estimated[' '] = mutableListOf(space)

            while (words.isNotEmpty() && estimated.any { it.value.size > 1 }) {
                val minsLenWords = words.minsBy { it.length }
                println("Длина: ${minsLenWords.first().length}")
                words.removeAll(minsLenWords.toSet())

                minsLenWords.forEach { word ->
                    val locAlph = word.toList().distinct()

                    alphabet.combinations(locAlph.size)
                        .map { locAlph.zip(it).toMap() }
                        .filter { bj ->
                            bj.all {
                                estimated[it.key]!!.contains(it.value)
                            }
                        }.forEach { miniBijection ->
                            //miniBijection['о']=='т'
                            val encoded = encodeWith(word, miniBijection)
                            if (!encoded.inDictionary()) {
                                val curPairIdxs =
                                    miniBijection.entries.map { varIdx(it.key, it.value) }
                                // (¬x->(¬y∧¬z))∧(¬y->(¬x∧¬z))∧(¬z->(¬y∧¬x)) => (¬x∨y)∧(¬x∨z)∧(x∨¬y)∧(x∨¬z)
                                curPairIdxs.forEach { x ->
                                    solver.addClause(VecInt(curPairIdxs.minus(x).plus(-x).toIntArray()))
                                }
                            }
                        }
                    solver.isSatisfiable
                    solver.primeImplicant().forEach { variable ->
                        if (variable < 0) {
                            val (k, v) = idxVar(-variable)
                            //estimated[k]!!.remove(v)
                            if (estimated[k]!!.size == 1) {
                                val encCh = estimated[k]!!.single()
                                println("$k однозначно $encCh")
                                // Теперь другие не могут быть им
                                val expIdxs = allPair.filter { it.second == encCh && it.first != k } //TODO
                                    .map { varIdx(it.first, it.second) }
                                expIdxs.forEach {
                                    solver.addClause(VecInt(listOf(-it).toIntArray()))
                                }
                ...
                println(estimated.entries.joinToString("\n"))
                println()
            }
            solver.isSatisfiable
            solver.primeImplicant().forEach { variable ->
                if (variable < 0) {
                    val (k, v) = allPair[-variable].first to allPair[-variable].second
                    estimated[k]!!.remove(v)
                } else println("ОГО!!!")
            }
            return estimated.entries.joinToString("\n")
        }
        return ""
    }*/
}
