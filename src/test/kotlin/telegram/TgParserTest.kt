...
internal class TgParserTest {

    /*@Test
    fun parseMessagesTest() {
        val tgParser = TgParser(File("data/chat.json"))
        val actual = tgParser.parseChat()

        val expected = Chat(
            178051894, "Дима Патока", "personal_chat", listOf(
                Message(
                    6364, "message", LocalDateTime.parse("2021-09-29T12:25:55"),
                    "o'Qaris", "user876213253", null, false,
                    text = Text(listOf(TextItem("ну так это для \"Расписания\"? 👀")))
                ),
                Message(
                    6366, "message", LocalDateTime.parse("2021-09-30T01:00:34"),
                    "Дима Патока", "user178051894", null,false,
                    text =
                ...
                Message(
                    ...
                    "Милка ❤", "user983918040", null,false,
                    text =
                ...
                Message(
                    ...
                    "o'Qaris", "user876213253", 110032,false,
                    text = Text(listOf(TextItem("Во ++")))
                ),
            )
        )
