...
fun main() {
    ...
    Combinatorics.cartesianProduct(
        ...
        .filter { variable ->
            listOf( // A -> B = (!A || B)
                // 1.1 - «Электрик и ты» не боевик или комедия
                variable.film != "ЭТ" || variable.genre !in listOf("Б", "К"),
                // 1.2 - Боевик, комедия и «Электрик и ты» получили 0, 1 и 3 награды
                !(variable.genre in listOf("Б", "К") || variable.film == "ЭТ")
                        || variable.award in listOf("0", "1", "3"),
                // 2.1 - «Жуя буррито» является комедией
                variable.film != "ЖБ" || variable.genre == "К",
                // 2.2 - «Жуя буррито» получила больше одной награды, а её зрительский рейтинг 7.2
        ...
        }.toList().combinations(n).filter { newVars ->
            // Все различны
            listOf<(FilmVar) -> String>({ it.film }, { it.genre }, { it.rating }, { it.award })
                .all { getter -> newVars.groupBy(getter).size == n }
        }.forEach { row ->
